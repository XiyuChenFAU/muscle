...
public:
    ...
    MX getnodelengthdisdiffall(Parm* parm, MX x, const std::vector<std::vector<double>>& jointposition, int musclenum, int use_p_variable, const std::vector<MX>& p_gamma_previous, const std::vector<MX>& p_mass_matrix);

    std::vector<MX> getnodedisdiff(muscle* Muscle, const std::vector<std::vector<MX>>& gammaallnode);
    std::vector<MX> getnodedisdiff(const std::vector<MX>& gammaprevious, const std::vector<std::vector<MX>>& gammaallnode);
    ...
    MX getnodedisdiffall(Parm* parm, MX x, const std::vector<std::vector<double>>& jointposition, int musclenum, int use_p_variable, const std::vector<MX>& p_gamma_previous, const std::vector<MX>& p_mass_matrix);
    std::vector<std::vector<std::vector<MX>>> rearrange_gamma_eta(Parm* parm, MX x, int musclenum);
    MX getobjective(Parm* parm, MX x, const std::vector<std::vector<double>>& jointposition, int musclenum, int use_p_variable, const std::vector<MX>& p_gamma_previous, const std::vector<MX>& p_mass_matrix);
