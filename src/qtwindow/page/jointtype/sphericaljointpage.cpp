    ...
    Jointpage(Jointpage)
{
    ...
    connect(modeGroup, QOverload<QAbstractButton*>::of(&QButtonGroup::buttonClicked), this, &sphericaljointpage::updateModeVisibility);

    QLineEdit *initialanglexedit=settextandlabel("initial rotation angle x","0", 0, 45, 140, 30, allfontsize);
    qedits.push_back(initialanglexedit);
    QLineEdit *initialangleyedit=settextandlabel("initial rotation angle y","0", 0, 125, 140, 30, allfontsize);
    qedits.push_back(initialangleyedit);
    QLineEdit *initialanglezedit=settextandlabel("initial rotation angle z","0", 0, 205, 140, 30, allfontsize);
    qedits.push_back(initialanglezedit);
    QLineEdit *axisxedit=settextandlabel("final angle x","0", 160, 45, 140, 30, allfontsize);
    qedits.push_back(axisxedit);
    QLineEdit *axisyedit=settextandlabel("final angle y","0", 160, 125, 140, 30, allfontsize);
    qedits.push_back(axisyedit);
    QLineEdit *axiszedit=settextandlabel("final angle z","0", 160, 205, 140, 30, allfontsize);
    ...
    if(index>jointnum-1){
        ...
        for(int i=0; i<3;i++){
            ...
            if(i<static_cast<int>(movementEdits.size())){
                if(rotationangle.size()>i && !rotationangle[i].empty()){
                    movementEdits[i]->setText(QString::fromStdString(movementToString(rotationangle[i])));
                }
                else{
...
bool sphericaljointpage::isSelfDesign() const
{
    return selfDesignRadio != nullptr && selfDesignRadio->isChecked();
}

std::vector<std::vector<std::vector<double>>> sphericaljointpage::getMoveSettingValue(double stepnum)
{
    if (isSelfDesign()) {
        customMoveSetting.resize(3);
    ...
    double initialangle1value=qedits[0]->text().toDouble();
    double initialangle2value=qedits[1]->text().toDouble(); 
    double initialangle3value=qedits[2]->text().toDouble();
    double angle1value=qedits[3]->text().toDouble();
    double angle2value=qedits[4]->text().toDouble(); 
    double angle3value=qedits[5]->text().toDouble();
    return {{{initialangle1value,angle1value,stepnum,0.0}},{{initialangle2value,angle2value,stepnum,0.0}},{{initialangle3value,angle3value,stepnum,0.0}}};
}

void sphericaljointpage::updateModeVisibility()
{
...
void sphericaljointpage::openMovementDesigner(int axisIndex)
{
    customMoveSetting.resize(3);
...
std::string sphericaljointpage::movementToString(const std::vector<double> &movement)
{
    std::ostringstream stream;
...
std::vector<std::vector<double>> sphericaljointpage::getMovementValue()
{
    if(!isSelfDesign()){
        return {};
    }
...
std::vector<double> sphericaljointpage::movementFromMoveSetting(const std::vector<std::vector<double>> &moveSettingAxis)
{
...
int sphericaljointpage::totalStepnum(const std::vector<std::vector<double>> &moveSettingAxis)
{
    int total = 0;
    for(const std::vector<double> &interval : moveSettingAxis){
        if(interval.size()>=3){
...
void sphericaljointpage::normalizeSelfDesignAxes()
{
    customMoveSetting.resize(3);
    int totalStep = 0;
